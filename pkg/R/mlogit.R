mlogit <- function(formula, data, subset, weights = NULL, na.action,
  ...
  m <- match(c("data","choice","shape","varying","sep",
               "alt.var","id.var","alt.levels","opposite"),
             names(mf), 0L)
  ...
  result <- list(coefficients = coef, logLik = logLik, logLik0 = logLik0,
                 ...
                 formula = formula, expanded.formula=expanded.formula,
                 model= mframe, index = index)
  class(result) <- "mlogit"
  result
}

...
alternatives. There are many useful applications of discrete choice
modelling in different fields of applied econometrics, using individual
data, which may be :
...
Suppose that we can define an index of satisfaction $V_j$ for each
alternative which depends linearly on cost ($x$) and price ($z$) :
...
In this case, the choice of alternative $j$ is increasing with
$V_j$. For sake of estimation, one has to transform $V_j$ which can
take any real value so that it belongs to the unit interval and can be
interpreted as a probability. The multinomial logit model is obtained
by applying such a transformation to the $V_j$s. More specifically, we
have :
$$
\left\{
\begin{array}{rcl}
...
\item a \emph{long} shape : in this case, there is one row for each
  ...
  for one each choice situation
\end{itemize}

This can be illustrated with two data sets. The first one,
\Rd{Fishing} comes with \proglang{mlogit}. The second one
\Rd{TravelMode} is from the \proglang{AER} package.
...
\va{income} and \va{size}. This advantage of this shape is
...
indicated or, in part, guessed by the \Rf{mlogit.data}
function. Here, we have 210 individuals which are indicated by a
variable called \va{individual}. The information about individuals
can also be guessed from the fact that the data frame is balanced
(every individual face 4 alternatives) and that the rows are ordered
...
choice can be indicated using the \Ra{chid.var}{mlogit.data} argument :
...
and dropped from the data using the \Ra{drop.index}{mlogit.data}
argument :
...
The final example is a data set called \Rd{Train} which contains
...
has to add an argument \Ra{id}{mlogit.data} which contains the
individual variable. The \Rat{index} attribute has now a
supplementary column, the individual index.
...
precisely, when working with multinomial logit models, one has to
...
the choice for one alternative. This means that, for example, we'll be
interested in the difference between the satisfaction index of two different
...
impact on utility than 10 mn in a car. In this case, alternative
specific coefficients are relevant. Monetary time is also alternative
specific, but in this case, one can consider than 1 euro is 1 euro
whatever it is spent in car or in public transports\footnote{At least
  if the monetary cost of using car is correctly calculated.}. In this
...
e^{-e^{\frac{\mu-z}{\theta}}} dz=e^{-e^{\frac{\mu-t}{\theta}}}
$$
...
The mean of $\epsilon_j$s is not identified if $U_j$ contains an
...
equal to one if individual $i$ made choice $j$ and 0 otherwise.
...
\item Approximate the function around $x_t$ using a second order
  \textsc{Taylor} series : $l(x)=f(x_t)+(x-x_t)g(x_t)+0.5
  (x-x_t)^2h(x_t)$ where $g$ and $h$ are the first two derivatives of
  $f$,
...
$$l(x)=f(x_t)+(x-x_t)g(x_t)+0.5 (x-x_t)'H(x_t)(x-x_t)$$
...
In a linear model, the coefficients can be directly interpretable as
...
respect with the explanatory variables, which can be be
...
\item For an individual specific variable, the sign of the coefficient
  ...
  coefficients for all the alternative, the weights being the
  probabilities of choosing the alternatives.
\end{itemize}
...
Consumer's surplus is a very simple expression with multinomial logit
...
\item \Ra{altsubset}{mlogit} indicates a subset on which the
  ...
  corresponds to the selected alternatives are used and all the
  observations which corresponds to choices for unselected
...
tests. A particular convenient syntax is provided in this case. 

For the likelihood ratio and the wald test, one can pass only the
fitted model as argument. In this case, it is guessed that the
hypothesis that the user wants to test is the homoscedasticity
hypothesis, \emph{i.e.}  \Rcl!sp.train=1! and \Rcl!sp.air=1!.
...
The second example we provide reproduces the first column of table
23.28 page 855 of \cite{GREEN:08}.
...
are very high, which means that they are poorly identified. This is
confirmed by the fact that the homoscedasticity hypothesis is not
rejected :

<<>>=
waldtest(hl.TM)
lrtest(hl.TM)
scoretest(ml.TM, heterosc = TRUE)
@ 
...
$W_m+\lambda_m I_m$ can be interpreted as the expected utility of
choosing the best alternative of the nest $m$, $W_m$ being the
...
The inclusive values links the two models. 
...
is that the determinist parts of the utility for each alternative is
not normalized by the nest elasticity :
...
\citet{KOPP:WEN:98} and \citet{HEIS:02}.
...
With overlapping nests, the notations are slightly modified :
...
$$
P_j= \sum_{l \mid j \in B_l} \frac{e^{V_j/\lambda_l}}{N_l}\frac{N_l^{\lambda_l}}{\sum_m N_m^{\lambda_m}}=\sum_{l \mid j \in B_l} P_{j\mid l}{P_l}
$$
...
somewhat meaningless, as it is related to the degree of correlation of
the alternatives within the nests and that there is only on
...
Recall that the unscaled version version of the nested logit model is
not compatible with the hypothesis of random utility maximisation. The
parameters associated with the inclusive values are far more difficult
to interpret in this case.

The second example deals with a choice of a heating mode. The data set
is called \Rd{HC}. There are five alternatives, four of them provide
...
We now estimate a nested logit model with two nests : the cooling /
non-cooling systems :
<<>>=
...
nl.HC <- mlogit(depvar~occa+icca+och+ich, HC, 
                nests = list(cooling = c('ecc', 'erc', 'gcc', 'hpc'), 
                  noncool = c('ec', 'gc', 'er')))
## nl <- mlogit(depvar~invcost+opcost, HC, 
##              nests = list(cooling = c('ecc', 'erc', 'gcc', 'hpc'), 
##                           noncool = c('ec', 'gc', 'er')))
summary(nl.HC)
@ 
...
of 0.7, which is quite high. The results of the three tests are given
bellow :
...
test equality between both elasticities. \Rf{linearHypothesis}
provides a particular convenient interface for that :

<<>>=
linearHypothesis(nl.HC, "iv.cooling = iv.noncool")
@ 

Therefore the hypothesis is not rejected and can be unforced by
updating the model with the argument \Ra{un.nest.el}{mlogit} equal to
\code{TRUE}.
...
A likelihood ratio test confirms that the hypothesis is not rejected :
...
distribution. Let $G$ be a function with $J$ arguments $y_j \geq 0$. $G$ has
...
For point 3, let denote\footnote{citation from Mc Fadden.} :
...
Often, especially in the case of stated preference survey, we have
repeated observations for the same individuals. In this case, this
panel dimension can be taken into account in the mixed logit
model. More specifically, we'll compute one probability for each
individual and this is these probabilities that are included in the
log-likelihood function. For a given vector of coefficients $\beta_i$,
the probability for alternative $k$ for the $l$th observation for the
individual $i$ :

$$P_{ikl}=\frac{e^{\beta_ix_{ikl}}}{\sum_j e^{\beta_ix_{ikj}}}$$
The probability for the chosen probability for the $l$the observation
for the individual $i$ is :
$$P_{il}=\prod_k {P_{ikl}}^{y_{ikl}}$$
Finally, the joint probability for the $K$ observations of individual
$i$ is :
$$P_{i}=\prod_i \prod_k {P_{ikl}}^{y_{ikl}}$$
...
$$
and $R$ draws from a Gumbell distribution are obtainde by computing
...
\includegraphics[width=\textwidth]{./graph/haltonvsrandom.pdf}
  \end{figure}
\end{center}
On this figure, one can see that, when using pseudo-random numbers, we
...
To obtain a couple of correlated coefficients, one has to
post-multiply a matrix of uncorrelated coefficients by the Cholesky
matrix.

If $\mbox{V}(\epsilon_1,\epsilon_2)=I$, then the variance of $
(\epsilon_1 \epsilon_2) C $ is $\Omega$
...
\right)
$$
Starting with two uncorrelated parameters $(\epsilon_1,\epsilon_2)$,
...
\Rob{mlogit} object, as second argument \Ra{par} the parameter(s) to
be extracted and as third argument \Ra{norm} the coefficient (if any)
that should be used for normalization. This is usually the coefficient
of the price (taken as a non random parameter), so that the effects
can be interpreted as monetary values. This function returns a
\Rob{rpar} object, and several methods/functions are provided to
describe it :

<<>>=
tvalue <- rpar(Train.mxlc, "time", norm = "price")
summary(tvalue)
med(tvalue)
mean(tvalue)
stdev(tvalue)
...
stdev(Train.mxlc)
@ 
The three tests can be applied to test the specification of the
model. Actually, three nested models can be considered :
\begin{itemize}
...
As all models estimated by maximum likelihood, three testing
...
\begin{itemize}
\item the Wald test is based only on the unconstrained model,
\item the Lagrange multiplier tests is based only on the constrained
  model,
\item the Likelihood ratio test is based on the comparison of both
  models.
\end{itemize}

The three principles of test are better understood using
figure~\label{fig:threetests}

\includegraphics{graph/threetests}
...
straight line on~\ref{fig:threetests} . The constrained model is just
$\hat{\theta}_{c}=\theta_o$, \emph{i.e.} the constrained model is not
estimated. The unconstrained model corresponds to the maximum of the
curve that represents the log-likelihood function.

\begin{itemize}
\item The Wald test is based on $f(\hat{\theta}_{nc})$ which is
  depicted by the arrow in~\ref{fig:threetests}. More generally, it is
  a vector of length $J$, whose expected value should be 0 if the
  hypothesis is true.
\item The Lagrange multiplier is based on the slope of the likelihood
  curve evaluated at the constrained model : $\frac{\partial \ln
    L}{\theta}(\hat{\theta}_c)$. Here again, this should be a random
  vector with expected value equal to 0 if $H_o$ is true.
...
\Rf{waldtest} et \Rf{lrtest}. The wald test is also implemented in
\Rf{linearHypothesis} from package \pkg{car} with a fairly different
syntax. We provide special methods of \Rm{waldtest}{mlogit} and
\Rm{lrtest}{mlogit} for \Rob{mlogit} objects and we also provide a
function for the lagrange multiplier (or score) test called
\Rf{scoretest}.

We'll see later on that the score test is especially usefull for
...
setting where the two models are indicated. This can done by passing
...
zero. The constrained model can be obtained by updated the previous
model :
...
<<eval = FALSE>>=
lrtest(m, . ~ . | . - income | .)
lrtest(m, mode ~ price | 1 | catch)
lrtest(mc, . ~ . | . + income | .)
lrtest(mc, mode ~ price | income | catch)
waldtest(m, . ~ . | . - income | .)
waldtest(m, mode ~ price | 1 | catch)
waldtest(mc, . ~ . | . + income | .)
waldtest(mc, mode ~ price | income | catch)
scoretest(mc, . ~ . | . + income | .)
scoretest(mc, mode ~ price | income | catch)
@ 

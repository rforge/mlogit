...
<<>>=
ml <- mlogit(choice ~ freq + cost + ivt + ovt | urban + income, Bhat, reflevel = 'car')
hl <- mlogit(choice ~ freq + cost + ivt + ovt | urban + income, Bhat, reflevel = 'car', heterosc = TRUE)
summary(hl)
@ 
...
<<>>=
library("car")
linearHypothesis(hl, c('sp.air=1', 'sp.train=1'))
@ 
...
ml <- mlogit(choice ~ wait + gcost + avinc, TravelMode, 
             reflevel = "car")
hl <- mlogit(choice ~ wait + gcost + avinc, TravelMode, 
...
<<>>=
c(coef(hl)[7:9], sp.car = 1)*pi/sqrt(6)
@ 
...
TravelMode <- mlogit.data(TravelMode,choice="choice",shape="long",
                          alt.var="mode",chid.var="individual")
TravelMode$avinc <- with(TravelMode,(mode=='air')*income)
nl <- mlogit(choice ~ wait + gcost + avinc, TravelMode, reflevel = "car", 
...
<<>>=
linearHypothesis(nl, "iv.cooling = iv.noncool")
@ 
...
Especially important for stated preference survey were several
...
\begin{itemize}
\item generate pseudo random-draws from a uniform distribution,
...
\Ra{rpar}{mlogit} argument to \Rf{mlogit}.
...
ml <- mlogit(choice ~ price + time + change + comfort, 
             Tr, panel = TRUE, rpar = c(time = "cn", change = "n", comfort = "ln"),
             correlation = TRUE, R = 20, tol = 10, halton = NA)
summary(ml)
@ 
...
extracted using the function \code{rpar} which take as first argument
a \code{mlogit} object, as second argument the parameter(s) to be
extracted and as third argument the coefficient that should be used
for normalization. This is usually the coefficient of the price (taken
as a non random parameter), so that the effects can be interpreted as
monetary values. This function returns a \code{rpar} object, and
several methods/functions are provided to describe it :

<<>>=
tvalue <- rpar(ml, "time", norm = "price")
summary(tvalue)
med(tvalue)
mean(tvalue)
stdev(tvalue)

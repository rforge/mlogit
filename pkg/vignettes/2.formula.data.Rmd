...
`Train`^[Used by @BENA:BOLD:BRAD:93 and @MEIL:ROUW:06.] is an example
  of a *wide* data set:
...
```{r label = 'index of the transoformed Train data set'}
head(index(Tr), 3)
``` 
...
To replicate published results later in the text (Section
\@ref(sec:modecanada)), we'll use only a subset of the choice
situations, namely those for which the 4 alternatives are
available. This can be done using the `subset` function with the
`subset` argument set to `noalt == 4` while estimating the model. This
can also be done within `mlogit.data`, using the `subset` argument.
...
hypothesis](./4.relaxiid.html)].  As the overall scale of utility is

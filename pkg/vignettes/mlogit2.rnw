...
<<Train.mxlu, echo = FALSE, results = 'hide'>>=
strt <- c(0.137351794146992, 4.30849574719798, 0.887994686551665, 
          2.45345140549618, 4.90794881965367, 1.6382548714579, 2.40096289296281)
Train.mxlu <- mlogit(choice ~ price + time + change + comfort | - 1, Tr,
                     panel = TRUE, rpar = c(time = "n", change = "n", 
                                            comfort = "n"), R = 100,
                     correlation = FALSE, halton = NA, method = "bhhh", start = strt)
@
...
<<Train.mxlc, echo = FALSE, results = 'hide'>>=
strt <- c(0.146661855513732, 4.89375230619496, 0.995435497561417, 2.66085236909391, 
           5.35219944579397, -0.0520907097201832, 1.76125615858177, 1.03843914364791, 
           0.730478464708962, 2.50670698404667)
Train.mxlc <- mlogit(choice ~ price + time + change + comfort | - 1, Tr,
                     panel = TRUE, rpar = c(time = "n", change = "n", 
                                            comfort = "n"), R = 100,
                     correlation = TRUE, halton = NA, method = "bhhh", start = strt)
@ 
...
<<Train.mxlc2, echo = FALSE, results = 'hide'>>=
strt <- c(0.149178038536331, 5.15159582558438, 1.10490245223184,
          2.75984080509624, 1.76784572945564, 5.57261582208262,
          1.19752704860657, 2.81936053678655)
Train.mxlc2 <- mlogit(choice ~ price + time + change + comfort | - 1, Tr,
                      panel = TRUE, rpar = c(time = "n", change = "n", comfort = "n"), R = 100,
                      correlation = c("time", "comfort"), halton = NA, method = "bhhh", start = strt)
@ 
...
lh.mxc <- linearHypothesis(Train.mxlc, 
                           c("chol.time:time = 0", "chol.time:change =  0",
                             "chol.change:change = 0", "chol.time:comfort = 0", 
...
<<mx.rt, echo = FALSE, results = 'hide'>>=
strt <- c(-0.0186807298038184, -0.103028409822579, 0.108497212141664, 
          0.142260749207574, -0.715731973680346, -0.149741650205133, -0.331424607943094)
